...
class AhoyDTUPlatform implements DynamicPlatformPlugin {
  ...
  private setupAccessory(accessory: PlatformAccessory, device: any) {
    ...
    // Try to use Energy Provider Service for power devices if available
    if (device.dataType === 'power' && this.isEnergyProviderServiceAvailable()) {
      this.log.info('🎉 Energy Provider Service verfügbar! Verwende native Apple Home Integration!');
      this.setupEnergyProviderService(accessory, device);
      return;
    }
  ...
  /**
   * Checks if Energy Provider Service is available (iOS 16+, HAP 2.0)
   */
  private isEnergyProviderServiceAvailable(): boolean {
    try {
      // @ts-ignore - Check if EnergyProvider service exists
      return !!this.Service.EnergyProvider;
    } catch (error) {
      return false;
    }
  }
  ...
  private setupEnergyProviderService(accessory: PlatformAccessory, device: any) {
    ...
    try {
      ...
      const energyProviderService = new this.Service.EnergyProvider(device.displayName);
      ...
      try {
        ...
        if (this.Characteristic.CurrentPower) {
          ...
          this.log.info('⚡ CurrentPower Characteristic hinzugefügt');
        }
      } catch (e) {
      ...
      try {
        ...
        if (this.Characteristic.DailyEnergy) {
          energyProviderService
            ...
            .onGet(() => {
              // Try to get daily energy from device data or use default
              const deviceData = this.deviceData.get(device.topic);
              if (deviceData && deviceData.dailyEnergy) {
                return parseFloat(deviceData.dailyEnergy) || 0;
              }
              return 0; // Default value
            });
          this.log.info('📊 DailyEnergy Characteristic hinzugefügt');
        }
      } catch (e) {
      ...
      try {
        ...
        if (this.Characteristic.TotalEnergy) {
          energyProviderService
            ...
            .onGet(() => {
              // Try to get total energy from device data or use default
              const deviceData = this.deviceData.get(device.topic);
              if (deviceData && deviceData.totalEnergy) {
                return parseFloat(deviceData.totalEnergy) || 0;
              }
              return 0; // Default value
            });
          this.log.info('🔋 TotalEnergy Characteristic hinzugefügt');
        }
      } catch (e) {
        this.log.warn('TotalEnergy Characteristic nicht verfügbar');
      }
      
      accessory.addService(energyProviderService);
  ...
  /**
   * Fallback method to setup standard service if Energy Provider fails
   */
  private setupStandardService(accessory: PlatformAccessory, device: any) {
    this.log.warn('⚠️ Fallback auf Standard-Service');
    ...
    // Setup standard service logic (copy from original setupAccessory)
    if (device.serviceType === this.Service.LightSensor) {
      service.getCharacteristic(this.Characteristic.CurrentAmbientLightLevel)
        .onGet(() => {
          if (this.isSystemOffline) {
